...
const generateAppPackageJson = packageJson =>
  Object.assign({}, fp.pick([`name`, `version`], packageJson), {
    main: `./main.js`
  })
...
const copyGaia = (buildPath, electronVersion, platform, arch, callback) => {
  ...
  fs.copy(
    path.join(__dirname, `../../../builds/ichain/${platformPath}_amd64`),
    `${buildPath}/bin`,
    callback
  )
}
...
const packagerWrapper = async ({ name, version }, options) => {
  const source = (await packager(options))[0]

  const destination = sanitizeAssetName(
    `${name}-v${version}-${platformNames[options.platform]}`
  )
...
const build = async platform => {
  ...
  const options = {
    ...
    out: path.join(__dirname, "../../../builds/icheck"),
    overwrite: true,
    packageManager: "yarn",
    platform
  }
...
const main = () =>
  cli(optionsSpecification, async options => {
    ...
    const gaiaVersionHash = await sha256(
      fs.createReadStream(path.join(__dirname, `ichain/COMMIT.sh`))
    )

    // If we're doing a local build then copy the specified network
    // configuration.
    fs.copySync(
      `src/main/networks/${path.basename(options.network)}`,
      `build/networks/${path.basename(options.network)}`,
    )
    ...
    // Rewrite config file.
    const configFile = path.join(__dirname, `../../src/main`, `config.toml`)
    const config = fs.readFileSync(configFile, { encoding: `utf8` })
    fs.writeFileSync(path.join(__dirname, `../../build/config.toml`), updateConfig(config, options))
